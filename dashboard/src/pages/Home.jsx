  ...
  const cargarDatosIniciales = useCallback(async () => {
    ...
    try {
      ...
      const [sucursalesRes, procesadoresRes, estadisticasRes, aclaracionesRes, ventasRes] = await Promise.all([
        api.get(`/sucursales-alerta`).catch(() => ({ data: [] })),
        api.get(`/cargos_auto/procesadores-alerta`).catch(() => ({ data: [] })),
        api.get(`/estadisticas-generales`).catch(() => ({
          data: { totalAclaraciones: 0, totalRecuperacion: 0, totalCargosAuto: 0, totalCaja: 0 }
        })),
        api.get(`/aclaraciones/dashboard`).catch(() => ({ data: { aclaracionesPorMes: [] } })),
        api.get(`/ventas/resumen?anio=2025`).catch(() => ({ data: [] }))
      ]);

      // Asegurarnos de que siempre tenemos arrays
      const sucursales = Array.isArray(sucursalesRes.data) ? sucursalesRes.data : [];
      const procesadores = Array.isArray(procesadoresRes.data) ? procesadoresRes.data : [];
      // Ensure ventas is an array
      let ventas = [];
      if (ventasRes && ventasRes.data) {
        ventas = Array.isArray(ventasRes.data) ? ventasRes.data :
                (typeof ventasRes.data === 'object' ? [ventasRes.data] : []);
      }

      setAlertaSucursales(sucursales);
      setAlertaProcesadores(procesadores);
      setEstadisticasGenerales(estadisticasRes.data || {});

      // Procesar datos de aclaraciones para el gráfico
      const aclaracionesPorMes = aclaracionesRes.data?.aclaracionesPorMes || [];
      setDatosAclaraciones(aclaracionesPorMes);

      // Datos reales de ventas mensuales 2025 para el gráfico de líneas
      const transformedData = ventas.map(item => ({
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="backdrop-blur-lg bg-white/10 rounded-xl shadow-2xl p-6 border border-white/20">
            ...
            <div className="space-y-3 max-h-80 overflow-y-auto custom-scrollbar">
              {(alertaSucursales && Array.isArray(alertaSucursales) && alertaSucursales.length > 0) ? (
