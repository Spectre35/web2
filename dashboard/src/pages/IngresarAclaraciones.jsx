...
export default function IngresarAclaraciones() {
  ...
  function manejarPegado(e) {
    ...
    const efevooHeaders = [
      "ID", "FOLIO", "FOLIO SIA", "CLIENTE", "CLIENTE", "SUCURSAL", "NÚMERO DE TARJETA", "NUMERO DE TARJETA", "MARCA DE TARJETA", "TIPO DE TARJETA", "MÉTODO DE PAGO", "METODO DE PAGO", "FECHA Y HORA", "MONTO", "NÚMERO DE AUTORIZACIÓN", "NUMERO DE AUTORIZACION", "AFILIACIÓN", "AFILIACION", "PRODUCTO"
    ];
    // Mapeo EFEVOO -> columnas internas (flexible con mayúsculas/minúsculas y acentos)
    const efevooToCol = {
      "ID": "ID_DE_TRANSACCION",
      "FOLIO": "ID_DE_TRANSACCION",
      "FOLIO SIA": "ID_DEL_COMERCIO_AFILIACION", // Mapear Folio SIA a ID del comercio
      "Folio SIA": "ID_DEL_COMERCIO_AFILIACION", // Mapear Folio SIA a ID del comercio
    ...
    if (paste.includes("DATOS DE LA TRANSACCIÓN") || paste.includes("No. caso:") || paste.includes("Afiliado Pagador:")) {
      ...
      // Detectar año de la fecha de venta, pero SIEMPRE usar mes corriente del sistema para MES_PETICION
      const fechaActual = new Date();
      const meses = ["ENERO", "FEBRERO", "MARZO", "ABRIL", "MAYO", "JUNIO", "JULIO", "AGOSTO", "SEPTIEMBRE", "OCTUBRE", "NOVIEMBRE", "DICIEMBRE"];

      if (newRow["FECHA_VENTA"]) {
        const { anio } = obtenerNombreMes(newRow["FECHA_VENTA"]);
        newRow["AÑO"] = anio || fechaActual.getFullYear().toString();
      } else {
        newRow["AÑO"] = fechaActual.getFullYear().toString();
      }

      // SIEMPRE usar el mes corriente del sistema para MES_PETICION
    ...
    if (rows.length >= 2) {
      ...
      const encabezadosClaveEFEVOO = ["ID", "Cliente", "Monto", "Fecha y Hora", "Folio SIA"];
          ...
          try {
            ...
            setFilas(prev => {
              ...
              if (filasEnriquecidas.length <= filasVacias) {
                ...
                console.log('✅ Suficientes filas vacías, llenando las existentes');
                const nuevasFilas = [...prev];
    ...
    // Detectar formato BSD vertical - POR NOMBRES CARACTERÍSTICOS
    // Headers únicos que identifican claramente un documento BSD
    const bsdUniqueHeaders = ["AFILIACION", "BIN TARJETA", "REFERENCIA", "IMPORTE"];

    let foundBsdHeaders = 0;
    let actualHeaderCount = 0;

    // Buscar headers BSD característicos en las primeras 15 filas
    if (rows.length >= 20) {
      for (let i = 0; i < Math.min(15, rows.length - 5); i++) {
        const header = rows[i]?.trim().toUpperCase();
        if (bsdUniqueHeaders.includes(header)) {
          foundBsdHeaders++;
        }
      }

      // Contar headers reales - paramos cuando encontremos el primer valor claramente numérico
      for (let i = 0; i < Math.min(15, rows.length); i++) {
        const item = rows[i]?.trim();
        if (item) {
          // Si es un número puro, contiene $, o parece un valor, paramos el conteo
          if (item.match(/^\d+$/) || item.includes('$') ||
              (item.match(/^\d/) && item.length < 8)) {
            actualHeaderCount = i;
            break;
          }
          actualHeaderCount = i + 1;
        }
      }
    }

    // Si encontramos al menos 3 headers únicos de BSD, es formato BSD
    if (foundBsdHeaders >= 3 && actualHeaderCount >= 8) {
      console.log(`📋 BSD detectado por nombres: ${foundBsdHeaders} headers únicos encontrados`);
      console.log(`📋 Total de headers detectados: ${actualHeaderCount}`);

      const actualHeaders = rows.slice(0, actualHeaderCount).map(r => r.trim().toUpperCase());
      const numRecords = Math.floor((rows.length - actualHeaderCount) / actualHeaderCount);
      console.log(`📋 Procesando ${numRecords} registros con headers:`, actualHeaders);

      const newRows = [];
      for (let i = 0; i < numRecords; i++) {
        const start = actualHeaderCount + i * actualHeaderCount;
        const reg = {};

        actualHeaders.forEach((h, idx) => {
          const valuePos = start + idx;
          let value = rows[valuePos] ? rows[valuePos].trim() : "";
          const col = bsdToCol[h] || mapHeaders[h];
          ...
          if (col && columnas.includes(col)) {
            ...
            reg[col] = value;
            console.log(`✅ ${h} -> ${col} = "${value}"`);
          } else if (col) {
            console.log(`⚠️ Header reconocido pero no en schema: ${h} -> ${col}`);
          } else {
            console.log(`⚠️ Header no reconocido: ${h}`);
          }
        });

        // Solo las columnas base
        ...
        if (row["FECHA_VENTA"]) {
          ...
          row["AÑO"] = anio || fechaActual.getFullYear().toString();
        } else {
      ...
      (async () => {
        try {
          ...
          setMensaje(`✅ Se pegaron ${newRows.length} filas BSD (detectado por nombres) y se buscaron los clientes automáticamente`);
        } catch (error) {
          ...
          setMensaje(`✅ Se pegaron ${newRows.length} filas BSD (detectado por nombres) - búsqueda automática falló`);
        }
        setTimeout(() => setMensaje(""), 4000);
      })();
    ...
    // Vertical alterno (clave en una línea, valor en la siguiente)
    if (rows.length > 4 && rows.length % 2 === 0) {
      // Verificar si parece formato alterno (clave-valor en líneas separadas)
      let esFormatoAlterno = true;
      for (let i = 0; i < Math.min(10, rows.length); i += 2) {
        if (i + 1 < rows.length) {
          const clave = rows[i].trim().toUpperCase();
          const valor = rows[i + 1].trim();
          // Las claves deberían ser texto (nombres de campos) y los valores datos
          if (!clave || !valor || /^\d+$/.test(clave)) {
            esFormatoAlterno = false;
            break;
          }
        }
      }
      ...
      if (esFormatoAlterno) {
        console.log('📋 Formato vertical alterno detectado (BSD)');
        const obj = {};
        for (let i = 0; i < rows.length; i += 2) {
          if (i + 1 < rows.length) {
            const key = rows[i].trim().toUpperCase();
            const value = rows[i + 1].trim();
            if (key && value) {
              const colInterno = mapHeaders[key] || key;
              obj[colInterno] = value;
            }
          }
        }
        ...
        if (obj['IMPORTE']) obj['MONTO'] = normalizarMonto(obj['IMPORTE']);
        ...
        // Solo las columnas base, nunca más ni menos
        const newRow = Object.fromEntries(columnas.map(col => [col, obj[col] || ""]));
        newRow["PROCESADOR"] = "BSD"; // Asumir que es BSD ya que usa este formato
        ...
        // Llenar las filas existentes primero
        setFilas(prev => {
          const filasVacias = prev.filter(fila => {
            const valoresConDatos = Object.entries(fila).filter(([key, value]) => {
              if (key === "EUROSKIN" && (value === "false" || value === "")) return false;
        ...
        setMensaje("✅ Se detectó formato vertical alterno BSD y se pegó 1 fila.");
        setTimeout(() => setMensaje(""), 4000);
        e.target.value = "";
        return;
      }
