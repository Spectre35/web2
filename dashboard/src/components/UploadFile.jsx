...
export default function UploadFile({ tabla }) {
    ...
    async (acceptedFiles) => {
      ...
      for (let i = 0; i < acceptedFiles.length; i++) {
        ...
        try {
          ...
          // Si es un error de columnas faltantes, mostrarlo inmediatamente
          if (err.response?.data?.columnasFaltantes) {
            const faltantes = err.response.data.columnasFaltantes;
            setMensaje(`❌ COLUMNAS FALTANTES en ${archivo.name}: ${faltantes.join(', ')}`);
            setSubiendoMultiple(false);
            return; // Detener la carga múltiple
          }
        }
      }
  ...
  const onDrop = useCallback(
    async (acceptedFiles) => {
      ...
      try {
        ...
        // Manejo específico para respuesta que indica columnas faltantes
        if (err.response?.status === 400 && err.response?.data?.columnasFaltantes) {
          const faltantes = err.response.data.columnasFaltantes;
          const encontradas = err.response.data.columnasEncontradas || [];
          const detalle = `❌ COLUMNAS FALTANTES: ${faltantes.join(', ')}\n\nEncabezados detectados: ${encontradas.join(', ')}`;
          setMensaje(detalle);
        }
        let mensajeError = `❌ Error al subir el archivo: ${err.response?.data?.error || err.message}`;

        // Manejo específico para errores de columnas faltantes en ventas
        if (err.response?.data?.columnasFaltantes) {
          const faltantes = err.response.data.columnasFaltantes;
          mensajeError = `❌ COLUMNAS FALTANTES: El archivo de ventas debe contener todas las columnas requeridas.\n\n` +
                        `📋 Columnas faltantes (${faltantes.length}):\n` +
                        `${faltantes.map(col => `• ${col}`).join('\n')}\n\n` +
                        `📋 Columnas encontradas en el archivo:\n` +
                        `${err.response.data.columnasEncontradas.map(col => `• ${col}`).join('\n')}`;
        }
  ...
  return (
    <div className="backdrop-blur-lg bg-white/10 p-6 rounded-xl shadow-xl flex flex-col items-center border border-white/20">
      ...
      {/* Botón para borrar mes corriente (solo para cargos_auto) */}
      {tabla === 'cargos_auto' && (
        <button
      ...
      {mensaje && (
        <div className="mt-4 text-center text-gray-200">
          <div className="text-sm bg-gray-800 border border-gray-600 rounded-lg p-3 whitespace-pre-line text-left max-w-lg">
            {mensaje}
          </div>
        </div>
      )}
