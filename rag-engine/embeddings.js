...
export class EmbeddingsEngine {
  ...
  constructor(options = {}) {
    ...
    this.batchSize = options.batchSize || 5; // Reducido para respuestas más rápidas
    this.maxRetries = options.maxRetries || 2; // Menos reintentos
    this.retryDelay = options.retryDelay || 500; // Menos delay
    this.cacheDir = options.cacheDir || './knowledge-base/embeddings';
    this.useCache = options.useCache !== false;
    
    // Configuración de batch processing optimizada
    this.concurrentBatches = options.concurrentBatches || 2; // Menos concurrencia para evitar saturar
    this.batchDelay = options.batchDelay || 50; // Menos delay entre batches
